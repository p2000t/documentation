...
cas_JumpTable:
        ...
        defw    cas_WriteEndOfTapeGap   ; e4 0b
...
cas_Status:
        ...
        rra			        ; bit 1 in carry 
        rra			        ; bit 2 in carry 
        rra			        ; bit 3 in carry  (write protect flag) 
        cpl			        ; Invert A
        bit 0,a			        ; Test Bit 4 (now in pos 0) if bit == 0 Z=>1  
        call restoreregs2		; 
        ret			        ; 
...
append_block:
        call write_Gap_Mark_Gap_Block   ; Wtite the mark and new block in one go
                                        ; write end of block gap
        ld e,KBIEN|FWD|WCD     		; KBIEN | Write | Forward 
        ld h,'a'	        	; with status 'a' 
        call status_out_wait_150ms
        ld a,(cassette_error)
        cp 0             		; all ok? 
        ret nz  			; no, abort
        ...
        call cas_WriteEndOfTapeGap      ; must now write end of tape marker (gap of 1.8 sec)
        ld a,(cassette_error)
...
; write error, D contains more info
        bit 4,d		                ; CIP
...
load_block:
        ...
        ld hl,end_read_cleanup	        ; the load routine ends wit RET, so we end up in cleanup.
...
prep_header:
        ...
        ld hl,(des1)		        ; header destination (0x6068)
        push hl			        ; on stack
        ld hl,read_payload		; 
        push hl			        ; data store routine on stack
...
wait_first_bit:
        ...
        and RDC|BET|CIP 	;  7T only bits 456 needed   (0xxx0000) clockbit, tapeok (1), cassetteejected(0)
...
innerwait1:
        ...
        djnz innerwait1		        ; 13 tstates if B not zero
                                        ; 1 loop takes 45 clocks when B == 0 we did 175 * 45 =  0,00315 seconds
        dec c			        ; 
...
skip_fwd_read_loop:
        call read_byte_rev		; read 8 data bit flips
...
rev_skip_byte_loop:
        ...
        jr nz,skip_rev_gap	        ; yes, no need to rev skip farther
